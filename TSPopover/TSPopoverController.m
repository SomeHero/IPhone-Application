...
- (id)init {
	if ((self = [super init])) {
        ...
        self.popoverBaseColor = [UIColor whiteColor];
...
{    
    ...
    CGPoint senderLocationInWindowPoint = [appWindow convertPoint:applicationFramePoint fromView:senderView];
...
- (void) showPopoverWithPoint:(CGPoint)senderPoint
{
    ...
    layer.shadowColor = [[UIColor colorWithRed:187/255.0 green:190/255.0 blue:197/255.0 alpha:1] CGColor];
...
- (CGRect) contentFrameRect:(CGRect)contentFrame senderPoint:(CGPoint)senderPoint
{
    ...
    if(self.arrowPosition == TSPopoverArrowPositionVertical){
        if(contentFrameRect.size.width > self.view.frame.size.width - (OUTER_MARGIN*2)){
